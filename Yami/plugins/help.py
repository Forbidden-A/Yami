...
import lightbulb
from lightbulb import HelpCommand, Context, filter_commands, get_help_text, Command, get_command_signature, Group, \
    plugins
...
class MyHelpCommand(HelpCommand):
    ...
    async def send_help_overview(self, context: Context) -> None:
        ...
        uncategorized_commands = await filter_commands(context, self.bot.commands.difference(set(all_plugin_commands)))
        ...
        for plugin, commands in plugin_commands:
            if not commands:
                continue
            text = f"{plugin}: "
            text += ", ".join(map(lambda command: f"`{command.name}`", sorted(commands, key=lambda c: c.name)))
    ...
    async def send_command_help(self, context: Context, command: Command) -> None:
        embed = hikari.Embed(
            ...
            text=f"Requested by {name(context)}",
            icon=context.author.avatar
        ).add_field(
            name="**Usage**",
            value=(
        ...
        if command_help := get_help_text(command):
            embed.add_field(name="**Description**", value=f"```md\n{command_help}```", inline=False)
